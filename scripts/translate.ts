...
async function fetchWithRetry(url: string, options: RequestInit, retries = MAX_RETRIES): Promise<Response> {
    try {
        ...
        if (retries > 0) {
            ...
            console.warn(`    ⚠️ ${isRateLimit ? 'Rate Limited' : 'Server Error'} (${response.status}). Retrying in ${backoff}ms...`);
            await delay(backoff);
        ...
        for (const model of MODELS) {
            try {
                ...
                if (!response.ok) {
                    ...
                    console.warn(`    ⚠️ Model ${model} failed: ${response.status} ${response.statusText}`);
                ...
                console.warn(`    ⚠️ Error with model ${model}:`, error instanceof Error ? error.message : error);
            }
        }
...
class POParts {
    static extractMsgid(block: string): string {
        ...
        lines.forEach(line => {
            ...
            const cleaned = line.replace(/^msgid\s+/, '').replace(/^"/, '').replace(/"$/, '');
            text += cleaned;
        });
...
async function processLocale(locale: string, translator: Translator): Promise<TranslationStats> {
    const filePath = path.join(LOCALE_DIR, `${locale}.po`);
    const stats: TranslationStats = { totalParsed: 0, translated: 0, skipped: 0, failed: 0 };
    ...
    while ((match = blockRegex.exec(content)) !== null) {
        ...
        console.log(`  📝 Translating: "${sourceText.substring(0, 40)}${sourceText.length > 40 ? '...' : ''}"`);
...
async function main() {
    ...
    const totalStats: TranslationStats = { totalParsed: 0, translated: 0, skipped: 0, failed: 0 };
